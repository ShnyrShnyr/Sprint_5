...
class TestRegistration:
    def test_success_reg(self, registration):
        ...
        assert (driver.find_element(*SignInLocators.EMAIL_FIELD).get_attribute("value") == login), "Error at login"  # проверяем, что значение поля value наша сгенерированная почта
        driver.quit()

        # Некорректный пароль при регистрации
    def test_fail_registration(self, registration, driver):
        registration(wrong_pass, driver)
